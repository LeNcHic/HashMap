...
namespace lab618
{
    ...
    */
    ...
    public:
        ...
        */
        // Добавляет в начало каждой ячейки Хеш-таблицы.
        bool add(T* pElement)
        {
          unsigned int idx;
          leaf* buff = findLeaf(pElement, idx);
          if (buff)
          {
            return false;
          }
        ...
        */
        ...
        {
          for (int i = 0; i < m_tableSize; ++i) {
            leaf* leaf_ = m_pTable[i];
            while (leaf_)
            {
              leaf* leaf_next = leaf_->pnext;
              delete leaf_;
              leaf_ = leaf_next;
            }
            m_pTable[i] = nullptr;
    ...
    private:
        ...
        */
        leaf *findLeaf(const T* pElement, unsigned int & idx)
        {
          ...
          while (buff)
          {
            if (Compare(buff->pData, pElement) == 0)
            {
              return buff;
            }
            buff = buff->pnext;
